...
If your project was set up with the shadcn CLI (`bunx shadcn@latest init`), you already have `scroll-fade`. It ships with the `shadcn` package, which the CLI imports in your global CSS file.
...
The [`ScrollArea`](/docs/components/scroll-area) and `MessageScroller` components can use `scroll-fade` on their scrollable viewport.
