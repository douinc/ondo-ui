...
| `--frame-panel-radius` | `calc(var(--frame-radius) - var(--frame-radius-inset) - 1px)` | The inner panel radius, reduced from `--frame-radius` so panel corners nest concentrically inside the frame.     |
