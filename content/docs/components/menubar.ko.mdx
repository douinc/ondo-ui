---
title: Menubar (메뉴바)
description: 데스크탑 애플리케이션에서 일반적으로 사용되는 시각적으로 지속되는 메뉴로, 일관된 명령어 집합에 빠르게 접근할 수 있게 합니다.
---
...
`MenubarCheckboxItem`을 사용하여 토글 가능한 옵션을 만듭니다.
...
`MenubarRadioGroup`과 `MenubarRadioItem`을 사용하여 단일 선택 옵션을 만듭니다.
...
중첩된 메뉴를 만들기 위해 `MenubarSub`, `MenubarSubTrigger`, `MenubarSubContent`를 사용합니다.
...
컴포넌트는 [Base UI Menubar](https://base-ui.com/react/components/menubar)를 래핑하며 모든 props를 허용합니다.
