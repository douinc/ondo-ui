...
**Recommended for new projects:** Use the shadcn CLI to initialize your framework, then register the `@ondo-ui` registry and install the theme.
...
For Laravel, start with `laravel new` before using `shadcn init`.
