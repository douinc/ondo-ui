...
Use the Ondo CLI to scaffold a new project, configure the registry, and install the initial theme directly from the terminal:
...
For Laravel, create the app first with `laravel new`, then run `npx @dou.so/ondo-ui@latest init -t laravel` from the app directory.
...
For Laravel, start with `laravel new` before using `npx @dou.so/ondo-ui@latest init`.
...
The Ondo CLI also forwards project and registry commands such as `search`,
`view`, `info`, `mcp`, `build`, and `registry` to shadcn. Use `--cwd <path>` to
