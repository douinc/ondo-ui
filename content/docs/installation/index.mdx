...
For Laravel, create the app first with `laravel new`, then run `npx ondo-ui@latest init -t laravel` from the app directory.
...
For Laravel, start with `laravel new` before using `ondo-ui init`.
